...
The digest has two sections. The first is **交易 Idea**: Newsletter/RSS, X, WSB and close movers all belong here, and only very clear company-specific Long/Short or bullish/bearish views qualify. Include as many source-grounded reasons as the source provides, including none; never invent reasons or balance them with an extra risk. The second is **产业变化**, reserved for the user's future AI-researcher sources and allowed to contain neutral technology, demand, product or ecosystem changes. Use only the public GitHub feed/context, never unrelated local files or other web pages.

## User experience contract

Render the digest with visible IDs: transaction-idea newsletters `N1`, transaction-idea X posts `X1`, industry changes `I1`, WSB posts `R1`–`R10`, and close movers `G1`/`L1`. WSB is always the WallStreetBets DD flair daily Top ten, whose rankings must be shown as DD #1 through #10 when available. Show tickers attached to each post; do not aggregate them into a separate heat list. If a post has saved `metadata.images`, show those local paths as part of the source material. End by telling the user they may ask to expand any ID. If they ask to expand an ID, locate that exact `display_id` in the latest context or stock-pool JSON and answer only from its stored source material.
